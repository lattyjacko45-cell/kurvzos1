...
import { runSophia, getLatestSophiaAdvice } from "@/lib/sophia/engine.server";
...
/** GET — most recent saved marketing read. Never triggers a model call. */
export async function GET() {
  ...
  const latest = await getLatestSophiaAdvice(ids.profileId);
  return NextResponse.json(latest);
}

/** POST — generate a fresh marketing read, optionally answering a question. */
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Sophia could not respond." },
      { status: 500 }
    );
  }
}
