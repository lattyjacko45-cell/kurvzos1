...
import { runMarcus, getLatestMarcusAdvice } from "@/lib/marcus/engine.server";
...
/** GET — most recent saved financial read. Never triggers a model call. */
export async function GET() {
  ...
  const latest = await getLatestMarcusAdvice(ids.profileId);
  return NextResponse.json(latest);
}

/** POST — generate a fresh financial read, optionally answering a question. */
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Marcus could not respond." },
      { status: 500 }
    );
  }
}
