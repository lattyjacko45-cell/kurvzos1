...
import { runRenee, getLatestReneeAdvice } from "@/lib/renee/engine.server";
...
/** GET — most recent saved strategy. Never triggers a model call. */
export async function GET() {
  ...
  const latest = await getLatestReneeAdvice(ids.profileId);
  return NextResponse.json(latest);
}

/** POST — generate a fresh strategic read, optionally answering a question. */
