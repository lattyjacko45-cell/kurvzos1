...
import { runOlivia, getLatestOliviaAdvice } from "@/lib/olivia/engine.server";
...
/** GET — most recent saved operations read. Never triggers a model call. */
export async function GET() {
  ...
  const latest = await getLatestOliviaAdvice(ids.profileId);
  return NextResponse.json(latest);
}

/** POST — generate a fresh operations read, optionally answering a question. */
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Olivia could not respond." },
      { status: 500 }
    );
  }
}
