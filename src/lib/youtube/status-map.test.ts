 ...
 * Run with:  npm test
 *
 * Uses Node's built-in test runner and type stripping, so this adds no
 * dependency to the project. That is only possible because `status-map.ts` is
 * deliberately free of Prisma, Next and `@/` alias imports — keep it that way.
 */
...
function facts(over: Partial<YouTubeVideoFacts> = {}): YouTubeVideoFacts {
  return {
    uploadStatus: null,
